...
async function fetchMovies(type) {
  const url = `https://api.themoviedb.org/3/search/movie?query=comedy&include_adult=true&language=en-US&page=1`;
  const options = {
    method: "GET",
    headers: {
...
export async function GenerateMovieSection() {
  ...
  .map(
      (movie) => { if (movie.backdrop_path || movie.poster_path) return `
